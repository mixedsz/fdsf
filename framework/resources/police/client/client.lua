...
-- Chat i think i fucked up the server side validation i guess we will see if it works when you try to set your state bag 
...
lib.registerMenu({
	...
	elseif selected == 2 then
		TriggerServerEvent("police:searchNearbyPlayer")
...
lib.addKeybind({
	...
	onReleased = function(self)
		...
		local hasPermission = lib.callback.await('police:checkPermission', false)
...
lib.registerMenu({
	...
	onSelected = function(selected, secondary, args)
		if args then 
			if DoesEntityExist(vehicleShop.current) then
				DeleteVehicle(vehicleShop.current)
				vehicleShop.current = nil
				local timeout = 0
				while DoesEntityExist(vehicleShop.current) and timeout < 50 do
					Wait(20)
					timeout = timeout + 1
				end
			end
	...
	elseif selected == 2 then
		...
		for i = 1, #Police.Vehicles do 
			for j = 1, #Police.Vehicles[i].ranks do 
				if string.lower(stateBag.job.grade_name) == Police.Vehicles[i].ranks[j] then 

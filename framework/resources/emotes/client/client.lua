local Config = Zen.Config.Emotes
...
function EmoteMenu.RemoveProps()
    ...
    for _, object in ipairs(objects) do
        if DoesEntityExist(object) then
            ...
            if distance <= 10.0 then
                -- Check if attached to the player
                if IsEntityAttachedToEntity(object, ped) then
                    table.insert(foundProps, object)
                end
            end
        end
    end
    
    -- Detach and delete all found props
    for _, prop in ipairs(foundProps) do
    ...
    -- Also clean up tracked props
    if EmoteMenu.PlayerProps and #EmoteMenu.PlayerProps > 0 then
        for i = 1, #EmoteMenu.PlayerProps do
            local prop = EmoteMenu.PlayerProps[i]
    ...
    -- Notify server to remove props (server-side cleanup)
    TriggerServerEvent('scully_emotemenu:removeProps')
    ...
    if data.Options and data.Options.Props then
        ...
        if propCount > 0 then
            ...
            for i = 1, propCount do
                ...
                -- Store whether this is the first prop (for PTFX sync)
                local isFirstProp = (i == 1) and (data.Options.Ptfx and data.Options.Ptfx.AttachToProp)
                
                -- Request prop from server (send emote command so server can lookup placement)
                -- Server will look up placement from config for security
                EmoteMenu.AddPropToPlayer(data.Command, prop.Name, prop.Bone, i, variant, isFirstProp)
            end
        end
    end
...
---@param emoteCommand string The emote command to lookup placement from config
---@param prop string Prop model name
---@param bone number Bone ID
---@param propIndex number Index of prop in the emote's props array
---@param variant number Texture variation
---@param needsPtfxSync boolean Whether this prop needs PTFX sync
---@return number object Entity handle (nil, object is created asynchronously)
function EmoteMenu.AddPropToPlayer(emoteCommand, prop, bone, propIndex, variant, needsPtfxSync)
    -- Request model loading first (still needed for client to see it properly)
    lib.requestModel(prop, 1000)
    
    -- Request server to create the object
    -- Server will look up placement from config based on emote command
    TriggerServerEvent('scully_emotemenu:createProp', emoteCommand, prop, bone, propIndex, variant, needsPtfxSync or false)
    
    -- Note: Object will be returned via the attachProp event handler
    -- We return nil here since the object isn't available immediately
    -- The attachment will happen asynchronously via the event
    return nil
end
...
function EmoteMenu.RemoveUnsupportedEmotes()
    ...
    local GameBuildNotCompatible = 0
...
RegisterNetEvent('scully_emotemenu:attachProp', function(networkId, bone, placement, variant, needsPtfxSync, playerId)
    if not networkId or networkId == 0 then
        return
    end
    
    local localPlayerId = GetPlayerServerId(PlayerId())
    if playerId and playerId ~= localPlayerId then
        print('[scully_emotemenu] Security: Attempted to attach prop for wrong player, ignoring')
        return
    end
    
    TriggerServerEvent('scully_emotemenu:requestAttach', networkId)
    
    local object = nil
    local attempts = 0
    while attempts < 50 do
        object = NetworkGetEntityFromNetworkId(networkId)
    ...
    if variant then
        SetObjectTextureVariation(object, variant)
    end
    ...
    if needsPtfxSync then
        TriggerServerEvent('scully_emotemenu:syncProp', networkId)
    end
end)


RegisterNetEvent('scully_emotemenu:propsRemoved', function()
    if EmoteMenu.PlayerProps and #EmoteMenu.PlayerProps > 0 then
        for i = 1, #EmoteMenu.PlayerProps do
            local prop = EmoteMenu.PlayerProps[i]
            if prop and DoesEntityExist(prop) then
...
CreateThread(function()
    ...
    for _, category in ipairs(categoriesWithProps) do
        if AnimationList[category] then
            for _, emote in ipairs(AnimationList[category]) do
                if emote.Options and emote.Options.Props then
                    for _, prop in ipairs(emote.Options.Props) do
                        if prop.Name then
                            propWhitelist[prop.Name] = true
                        end
                    end
                end
            end
    ...
    for propName, _ in pairs(propWhitelist) do
        whitelistArray[#whitelistArray + 1] = propName
    end
    
    -- Send to server via server event
    TriggerServerEvent('scully_emotemenu:setPropWhitelist', whitelistArray)
end)

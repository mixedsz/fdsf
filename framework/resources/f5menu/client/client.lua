...
local function LoadSavedOutfits()
    ...
    for _, v in ipairs(outfits) do 
        outfitValues[#outfitValues + 1] = { label = v.name:upper(), value = v }
    end

    if #outfitValues == 0 then return Zen.Functions.Notify('You Do Have Any Saved Outfit!', 'x', '#009DFF') end 
    lib.registerMenu({
        id = 'f5SavedClothing',
        title = 'F5 Menu',
        position = 'bottom-right',
        description = 'Browse all available f5 options.',
        icon = 'shirt',
        options = {{ icon = "shirt", label = "Saved Outfits", close = false, values = outfitValues }},
        onClose = function(keyPressed) 
            lib.showMenu('f5Menu')
        end,   
    }, function(selected, scrollIndex)
        local selectedOutfit = outfitValues[scrollIndex]

        if selectedOutfit then
            local uniform = {
                name = selectedOutfit.value.name,
                model = selectedOutfit.value.model,
                components = selectedOutfit.value.components,
                props = selectedOutfit.value.props,
                disableSave = true,
            }
    ...
    if args and args.menu then 
        if args.role then 
            if Zen.Functions.IsRolePresent(args.role) then
                ...
                Zen.Functions.Notify('You Do Not Have Access To This Menu!', 'x', '#009DFF')
            end
        else
            ...
            elseif args.menu == 'f5SavedClothing' then
                LoadSavedOutfits()
            end

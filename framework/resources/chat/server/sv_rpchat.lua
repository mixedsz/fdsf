...
function sendMsg(firstline, msg, to) 
	TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.41vw; margin: -0.1vw; background-color: rgba(0, 0, 0, 0.6); width = auto; border-radius: 23px;"><i class="fas fa-globe"></i> {0} {1}</div>',
		args = { firstline , msg}
	})
end 
	...
	if not string.find(args[1], "/") and setContains(roleTracker, GetPlayerIdentifiers(source)[1]) and 
		...
		if not dontSend then
			...
			if sendBlockMessages then 
				...
				TriggerEvent('DiscordBot:ToDiscord', 'https://discord.com/api/webhooks/982787275010244608/UFkRaEi_Fr4ovagO85nUdgNQchtfmh00A5jlFwWGSB1TGzztplIJwLibWUYEiUavwJYS', GetPlayerName(source) .. ' [ID: ' .. source .. '](' ..steamhex.. ')', '```'..finalMessage..'```', AvatarURL, false, Source, TTS) --Sending the message to discord
			else 
				TriggerClientEvent('chatMessage', -1, roleStr .. name .. "^7 " .. finalMessage);
			end 
		end
...
RegisterCommand('tweet', function(source, args, rawCommand)
	...
	fal2 = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
    ...
    TriggerEvent('DiscordBot:ToDiscord',  'tweetadme', 'Tweet', '```ID:'..source..' '.. GetPlayerName(id) .. ' ['..fal2..'] ('..steamhex..') | Said:'..msg..'```', 'IMAGE_URL', true)
end, false)
 ...
 RegisterCommand('ad', function(source, args, rawCommand)
	...
	fal2 = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
		template = '<div style="padding: 0.41vw; margin: -0.1vw; background-color: rgba(255, 222, 0, 0.6); border-radius: 3px;"><i class="fas fa-ad"></i> {0}: {1}</div>',
        args = { fal, msg }
    })
            TriggerEvent('DiscordBot:ToDiscord', 'tweetadme', 'Advertisement', '```ID:'..source..' '.. GetPlayerName(id) .. ' ['..fal2..'] ('..steamhex..') | Said: '..msg..'```', 'IMAGE_URL', true)
end, false)
